...
logoutBtn.onclick = async()=>{ await supabase.auth.signOut(); loginPanel.classList.remove('hidden'); dashboard.classList.add('hidden'); logoutBtn.classList.add('hidden'); };
...
async function uploadImage(file){
  ...
  const { error } = await supabase.storage.from('menu-images').upload(filename, file, { upsert:false });
...
$('saveBtn').onclick = async()=>{
  ...
  try{
    ...
    const res = id ? await supabase.from('menu_items').update(payload).eq('id', id) : await supabase.from('menu_items').insert(payload);
...
async function deleteItem(id){ if(!confirm('Delete this item?'))return; const {error}=await supabase.from('menu_items').delete().eq('id',id); if(error) alert(error.message); await loadItems(); }
...
$('importBtn').onclick=async()=>{ if(!confirm('Import starter menu items?'))return; const rows=starterMenu.map((x,i)=>({...x, sort_order:i+1})); const {error}=await supabase.from('menu_items').insert(rows); if(error) alert(error.message); await loadItems(); };
init();
